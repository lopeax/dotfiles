...
if [ ! -d .git ]; then
    ...
    git init -q
    git remote add origin git://github.com/lopeax/dotfiles.git
    git remote set-url --push origin git@github.com:lopeax/dotfiles.git
    git fetch -q origin
...
if [ -f ~/.ssh/tony.pub ]; then
    sudo cp -f ~/.ssh/tony.pub ~root/.ssh/authorized_keys
fi
